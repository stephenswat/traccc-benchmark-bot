...
void process_teff(TEfficiency * eff, std::string tree, std::string out_dir) {
  ...
  myfile << "bin_left,bin_right,ntotal,efficiency,err_low,err_high" << std::endl;

  for (int i = 1; i <= num_bins; ++i) {
    myfile << hist->GetBinLowEdge(i) << "," << (hist->GetBinLowEdge(i) + hist->GetBinWidth(i)) << "," << hist->GetBinContent(i) << "," << eff->GetEfficiency(i) << "," << eff->GetEfficiencyErrorLow(i) << "," << eff->GetEfficiencyErrorUp(i) << std::endl;
...
void process_profile(TProfile * hist, std::string tree, std::string out_dir) {
  ...
  for (int i = 1; i <= num_bins; ++i) {
    myfile << hist->GetBinLowEdge(i) << "," << (hist->GetBinLowEdge(i) + hist->GetBinWidth(i)) << "," << hist->GetBinContent(i) << "," << hist->GetBinErrorLow(i) << "," << hist->GetBinErrorUp(i) << std::endl;
