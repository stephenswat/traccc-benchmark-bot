...
def main():
    ...
    subprocess.run(
        [
            ...
            "-e=25",
            "--num-sm=64",
            "--num-threads-per-sm=1536",
            "--from=%s~" % (str(mb.hexsha)),
            "--to=%s" % (pr.head.sha),
    ...
    df_merge = pandas.merge(
        df_before, df_after, how="outer", on="kernel", suffixes=("_before", "_after")
    )
    ...
    for i in df_merge.iloc:
        kernels.append(
            (
                i["kernel"],
                i["rec_throughput_before"],
                i["rec_throughput_after"],
                i["latency_before"],
                i["latency_after"],
            )
        )
    ...
    text += '<tr><th align="center">{0}</th><th align="center">{1}</th><th align="right">Delta</th><th align="center">{0}</th><th align="center">{1}</th></tr></thead>'.format(
    ...
    for k, b, a, lb, la in kernels:
        ...
        pa = la / a
        text += (
            '<tr><td><code class="notranslate">%s</code></td><td align="right">%s</td><td align="right">%s</td><td align="right">%s</td><td align="right">%.2f</td><td align="right">%.2f</td></tr>'
            % (k, render_time(b), render_time(a), delta, pb, pa)
        )

    text += (
        '<tr><td><strong>Total</strong></td><td align="right"><strong>%s</strong></td><td align="right"><strong>%s</strong></td><td align="right"><strong>%.1f%%</strong></td><td align="right"><strong>%.2f</strong></td><td align="right"><strong>%.2f</strong></td></tr>'
        % (
            render_time(total_before),
            render_time(total_after),
            (100.0 * (total_after - total_before) / total_before),
            total_lat_before / total_before,
